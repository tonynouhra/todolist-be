...
This module contains comprehensive unit tests for the security and authentication components,
including Clerk authentication and token validation.
"""
...
class TestClerkAuthenticator:
    """Test cases for ClerkAuthenticator."""
    ...
    async def test_verify_token_malformed_jwt(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await auth.verify_token("not.a.valid.jwt.token")
    ...
    async def test_verify_token_generic_exception(self):
        ...
        with patch('app.core.security.jwt.decode') as mock_decode:
            ...
            assert exc_info.value.status_code == status.HTTP_401_UNAUTHORIZED
            assert "Invalid authentication token: Unexpected error" in str(exc_info.value.detail)

    @pytest.mark.asyncio
    async def test_verify_token_with_minimal_payload(self):
        """Test token verification with minimal valid payload."""
        test_token = jwt.encode(
            {"sub": "user_123"},
            "secret",
            algorithm="HS256"
        ...
        assert result["sub"] == "user_123"
        assert "email" not in result  # Optional field

    @pytest.mark.asyncio
    async def test_verify_token_with_extra_claims(self):
        """Test token verification with extra claims."""
        test_token = jwt.encode(
            {
                "sub": "user_123",
                "email": "test@example.com",
    ...
    def test_authenticator_singleton_behavior(self):
        """Test that multiple instances use same settings."""
        with patch('app.core.security.settings') as mock_settings:
            mock_settings.clerk_api_url = "https://test.clerk.com"
            mock_settings.clerk_secret_key = "test_secret_key"
            
            auth1 = ClerkAuthenticator()
            auth2 = ClerkAuthenticator()
            
            assert auth1.clerk_api_url == auth2.clerk_api_url
            assert auth1.secret_key == auth2.secret_key
```

```python:/Users/tonynouhra/Documents/MyProjects/TodoList/todolist-be/tests/unit/test_pagination.py
"""
