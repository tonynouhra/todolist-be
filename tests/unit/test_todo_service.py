...
class TestTodoService:
    ...
    @pytest.mark.asyncio
    async def test_create_todo_sqlalchemy_error_handling(self, test_db, test_user):
        """Test SQLAlchemy error handling in create_todo."""
        service = TodoService(test_db)
    ...
    async def test_get_todos_with_ai_generated_filter(self, test_db, test_user):
        ...
        # Test filtering for manual todos
        manual_filter = TodoFilter(ai_generated=False)
        result = await service.get_todos_list(test_user.id, manual_filter, pagination)
        assert result["total"] >= 1
