...
class TestAIService:
    ...
    async def test_generate_subtasks_success(
        ...
        with patch("app.domains.ai.service.genai"):
            with patch("app.domains.ai.service.settings") as mock_settings:
                ...
                with patch.object(
                    service, "_generate_content_async", return_value=mock_response
                ):
                    with patch.object(service, "_store_interaction"):
                        request = SubtaskGenerationRequest(
                            todo_id=test_todo.id, max_subtasks=3
                        )
    ...
    async def test_generate_subtasks_quota_exceeded(
        self, test_db, test_user, test_todo
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_subtasks_content_filter(
        self, test_db, test_user, test_todo
    ):
    ...
    async def test_analyze_file_success(
        self, test_db, test_user, sample_file_analysis_response
    ):
        ...
        with patch("app.domains.ai.service.genai"):
            with patch("app.domains.ai.service.settings") as mock_settings:
                ...
                with patch.object(
                    ...
                    with patch.object(
                        service, "_get_file_by_id", return_value=mock_file
                    ):
                        with patch.object(service, "_store_interaction"):
                            file_id = uuid.uuid4()
                            request = FileAnalysisRequest(
                                file_id=file_id, analysis_type="summary"
                            )
                            ...
                            assert (
                                result.summary
                                == sample_file_analysis_response["summary"]
                            )
    ...
    async def test_get_service_status_healthy(self, test_db):
        ...
        with patch("app.domains.ai.service.genai"):
            with patch("app.domains.ai.service.settings") as mock_settings:
                ...
                with patch.object(
                    service, "_generate_content_async", return_value="OK"
                ):
    ...
    def test_build_subtask_generation_prompt(self, test_db, test_todo):
        ...
        with patch("app.domains.ai.service.genai"):
            with patch("app.domains.ai.service.settings") as mock_settings:
                ...
                prompt = service._build_subtask_generation_prompt_from_todo(
                    test_todo, 5
                )
    ...
    def test_parse_file_analysis_response_success(
        self, test_db, sample_file_analysis_response
    ):
    ...
    async def test_store_interaction_database_error(self, test_db, test_user):
        ...
        with patch("app.domains.ai.service.genai"):
            with patch("app.domains.ai.service.settings") as mock_settings:
                ...
                with patch.object(
                    test_db, "commit", side_effect=SQLAlchemyError("DB Error")
                ):
