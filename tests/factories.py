...
async def create_todo_with_subtasks(session, user_id: uuid.UUID, num_subtasks: int = 3, project_id: Optional[uuid.UUID] = None) -> tuple[Todo, list[Todo]]:
...
async def create_mixed_status_todos(session, user_id: uuid.UUID, project_id: Optional[uuid.UUID] = None) -> list[Todo]:
