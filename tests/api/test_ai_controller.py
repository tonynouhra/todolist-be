...
from app.schemas.ai import SubtaskGenerationResponse, FileAnalysisResponse, GeneratedSubtask
...
class TestAIController:
    ...
    @pytest.mark.asyncio
    async def test_generate_subtasks_success(self, authenticated_client: AsyncClient, test_todo, sample_subtask_response):
        ...
        with patch('app.domains.ai.service.AIService.generate_subtasks', return_value=mock_response):
    ...
    async def test_analyze_file_nonexistent(self, authenticated_client: AsyncClient):
        ...
        from app.exceptions.ai import AIInvalidRequestError
        with patch('app.domains.ai.service.AIService.analyze_file', 
    ...
    async def test_get_ai_service_status_healthy(self, authenticated_client: AsyncClient):
        ...
        with patch('app.domains.ai.service.AIService.get_service_status', return_value=mock_status):
    ...
    async def test_ai_endpoints_unauthorized_access(self, client: AsyncClient, test_todo):
        ...
        endpoints_data = [
            ("POST", "/api/ai/generate-subtasks", {"todo_id": str(test_todo.id), "max_subtasks": 3}),
            ("POST", "/api/ai/analyze-file", {"file_id": str(uuid.uuid4()), "analysis_type": "summary"}),
    ...
    async def test_ai_request_logging_integration(self, authenticated_client: AsyncClient, test_todo):
        ...
        with patch('app.domains.ai.service.AIService.generate_subtasks', return_value=mock_response) as mock_service:
            response = await authenticated_client.post("/api/ai/generate-subtasks", json=request_data)
